  ...
  <nav class="text-white mb-3">
    <div class="max-w-8xl py-2 sm:mx-8 mr-2">
      <div class="flex justify-between flex-col sm:flex-row">
        <div>
          <!-- logo -->
          <a class="flex items-center cursor-pointer" v-clipboard:copy="copy" href="/">
            <img src="../../assets/favicon.svg" class="w-12 rounded-full object-cover mr-2" alt="logo" />
            <span class="text-3xl font-bold">BeFake</span>
          </a>
        </div>
        <a href="#" @click="hideNavbar = !hideNavbar"
        ...
        <!-- links -->
        <div class="items-end space-x-2 flex flex-col sm:flex sm:flex-row p-[.5rem]" :class="{
          hidden: hideNavbar,
        }">
          <a class="cursor-pointer text-red-500 sm:py-2 py-[0.6px] sm:px-3 rounded-md font-bold"
            href="https://ko-fi.com/rahulvaidun">
            Donate
          </a>
          <a class="cursor-pointer text-white sm:py-2 py-[0.6px] sm:px-3 rounded-md font-bold" href="/map">
            Map
          </a>
          <a class="cursor-pointer text-white sm:py-2 py-[0.6px] sm:px-3 rounded-md font-bold" href="/about">
            About
          </a>
          <a class="cursor-pointer text-white sm:py-2 py-[0.6px] sm:px-3 rounded-md font-bold"
            @click="this.$store.commit('logout')" v-if="this.$store.state.loggedIn">
            Logout
          </a>
        </div>
      </div>
